...
const SearchAndFilters = ({
  ...
  const renderButton = btn => <button className="flex items-center text-sm lg:text-lg font-medium justify-center gap-2 w-fit lg:w-46 h-[63px] px-5 py-3.5 xl:px-6 xl:py-4.5 rounded-[10px] bg-Purple-60 text-white hover:bg-gradient-to-br 
      ...
      hover:bg-[position:100%_100%]" onClick={() => {
    const searchInput = document.getElementById(searchField.id);
    const searchTerm = searchInput?.value.trim() || "";
    const selectValues = formFields.reduce((acc, field) => {
      if (field.type === "select") {
        const el = document.querySelector(`select[name="${field.id}"]`);
    ...
    onSearch?.(searchTerm, selectValues);
      ...
      {btn.icon && <img src={btn.icon.toString()} alt="search icon" className="w-5 h-5 lg:w-6 lg:h-6" />}
      <span className="min-w-max hidden lg:flex text-sm xl:text-lg font-medium ">
        {btn.text}
      </span>
    </button>;
  return <div className="flex flex-col gap-y-5 lg:gap-y-0 " data-aos="zoom-in">
            <div className=" mx-auto w-full lg:max-w-[81.45363%] ">
                <div className="relative w-full  h-[80px] xl:h-[103px]">
                    <input type={searchField.type} id={searchField.id} placeholder={searchField.placeholder} className="pl-3.5 md:pl-5 xl:pl-6 w-full h-full text-base md:text-xl rounded-xl  lg:rounded-b-none border border-Grey-15 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-[10px] p-3.5 rounded-xl bg-Grey-10">
        {formFields.map(field => field.type === "select" && <SearchSelect key={field.id} field={{
        ...field,
