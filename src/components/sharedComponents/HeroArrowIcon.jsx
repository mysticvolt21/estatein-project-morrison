...
export default function HeroArrowIcon() {
  ...
  return <div ref={circle} className={`relative border border-Purple-95 dark:border-Grey-15 w-29.5 h-29.5 md:w-33 md:h-33 xl:w-44 xl:h-44 
          flex items-center justify-center rounded-full dark:bg-Grey-08 bg-Purple-75 overflow-hidden hover-glow
          drop-shadow-[0_0_6px_#a685fa] dark:drop-shadow-none backdrop-blur-lg ${isPulsing ? "animate-pulse-once" : ""}`} onClick={() => setIsPulsing(true)}>
        <div className="absolute w-full h-full rounded-full animate-rotate">
          {text.split("").map((char, i) => <span key={i} className="absolute left-1/2 text-[10.03px] md:text-[11.06px] xl:text-[15px] font-semibold 
              z-10 p-1.5 md:py-2 xl:py-3 text-animate" style={{
