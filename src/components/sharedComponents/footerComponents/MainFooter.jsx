...
function FooterColumn({
  ...
  return <div className="flex flex-col justify-items-start gap-4 md:gap-6 xl:gap-7.5 w-full h-fit pb-5 border-b border-b-Grey-15 md:border-0">  
      <h3 className="text-base md:text-lg xl:text-xl leading-6 font-medium
      ...
      <ul className="space-y-2 md:space-y-4 xl:space-y-5">
        {links.map((item, idx) => <li key={idx}>
            <Link to={item.link} className="group relative inline-block text-sm md:text-base xl:text-lg leading-6 font-medium
                        ...
                        ">
              {item.name}
              <span className="absolute left-0 -bottom-1 h-[2px] w-0 rounded-l-full opacity-0
...
function MainFooter({
  ...
  const services = navigationColumn.right.find(col => col.title === "Services");
  const properties = navigationColumn.left.find(col => col.title === "Properties");
  const contact = navigationColumn.left.find(col => col.title === "Contact Us");
  return <div className="relative flex flex-col lg:flex-row px-4 py-12.5 md:px-20 md:py-20 xl:px-40.5 xl:py-25 gap-12.5 md:gap-20">
      ...
      <div className="flex flex-col gap-5 md:gap-6 xl:gap-7.5 w-full lg:w-[26%] z-20">
        ...
        <form className="focus-within:border focus-within:border-Purple-75/30 relative group px-5 py-3.5 border border-Grey-15 bg-Grey-08 rounded-lg flex items-center justify-between gap-1.5 xl:gap-2.5">
          <span className="pointer-events-none absolute -inset-[2px] rounded-lg opacity-0 transition-opacity duration-300 group-focus-within:opacity-100">
            <span className="absolute inset-0 rounded-[inherit] blur-[5px]
                 bg-[linear-gradient(-145deg,#a685fa,#703bf7cc,#fa85eadd,#703bf7cc,#a685fa,#8254f8,#a685fa)]
                 bg-[length:200%_200%] animate-[gradient-move_10s_ease-out_infinite]"></span>
          ...
          <div className="w-[90%] flex items-center gap-1.5 xl:gap-2.5 relative z-10">
            ...
            <input type="text" placeholder="Enter Your Email" className="w-full bg-transparent placeholder:text-sm xl:placeholder:text-lg placeholder:text-Grey-60 placeholder:font-medium leading-6 focus:outline-0" />
          </div>
