...
function PropertiesCard({
  ...
  return <div className=" w-full h-full  bg-Grey-08 border border-Grey-15 p-6 md:p-7.5 xl:p-10 rounded-xl grid  transition-transform duration-700 ease-in-out" style={{
    perspective: "800px",
    ...
    gridTemplateAreas: "front"
      ...
      zIndex: flipped ? 1 : 0
                            ...
                            {!showInfo && <div className="py-1.5 px-3 xl:py-2 xl:px-3 bg-Grey-10 rounded-[28px] border border-Grey-15 max-w-max">
                            ...
                            <div>
                                <div>
                                    ...
                                    <p className="font-medium text-sm md:text-base xl:text-lg text-Grey-60">
                                        {displayText}
                                        {isLong && <span className="font-medium text-sm md:text-base xl:text-lg text-White underline cursor-pointer ml-1" onClick={() => setFlipped(true)}>
                                            Read More
                                            </span>}
                                    </p>
                                </div>
                            ...
                            {showInfo && <div className="flex gap-1.5 xl:gap-2.5 flex-wrap">
                                    {details?.map((detail, index) => <InfoComponent key={index} icon={detail.icon} desc={detail.label} padding="py-1.5 px-3.5 xl:py-2" />)}
                                </div>}
                        ...
                        <div className="flex flex-col justify-between h-full " style={{
      gridArea: "front",
      gridRow: " 1 / -1",
      gridColumn: "1 / -1",
      backfaceVisibility: "hidden",
