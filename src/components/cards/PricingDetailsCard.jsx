...
function PricingDetailsCard({
  ...
  return <div className="p-5 md:p-10 xl:p-12.5 border border-Grey-15 rounded-xl ">
        ...
        return <div key={index} className={`w-full ${itemWidthClass}
                            ${!isLastRow ? "border-b border-Grey-15 py-5 md:py-7.5 xl:py-10" : "pt-5 md:pt-7.5 xl:pt-10 "}
                        `}>
                        <Additional title={data.label} price={data.value} desc={data.description} className={`${PricingItem.length > 2 && index % 2 === 1 ? "lg:border-l border-Grey-15 lg:pl-5 " : ""}`} data-aos="fade-right" data-aos-delay={index * 50} />
                    </div>;
      })}
            </div>
