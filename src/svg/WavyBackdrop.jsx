...
export default function WavyBackdrop({
  ...
  const colors = darkMode ? ["#1A1A1A", "#A685FA4d", "#333333", "#262626"] : ["#ede7fe", "#ede7fe", "#a685fa4d", "#dbcefd"];
  ...
  return <div style={{
    ...
    height: "100%"
      ...
      <svg ref={svgRef} width="100%" height="100%" viewBox={viewBox} preserveAspectRatio="none" style={{
      overflow: "visible"
    }}>
        <defs>
          {paths.map((_, i) => <linearGradient key={`grad-${i}`} id={`gradAnimated-${i}`} x1="100%" y1="0%" x2="0%" y2="0%">
              {colors.map((color, idx) => <stop key={idx} offset={`${idx / (colors.length - 1) * 100}%`} stopColor={color}>
                  <animate attributeName="stop-color" values={gradientValues} dur={`${colorAnimationSpeed}s`} begin={`${i * delayBetweenPaths}s`} repeatCount="indefinite" />
                </stop>)}
              <animateTransform attributeName="gradientTransform" type="translate" from="1 0" to="-1 0" dur={`${waveMoveSpeed}s`} begin={`${i * delayBetweenPaths}s`} repeatCount="indefinite" />
            </linearGradient>)}

          {hoverGlow && <filter id="neonGlow" x="-50%" y="-50%" width="200%" height="200%">
        ...
        {paths.map((d, i) => <path key={i} d={d} fill="none" stroke={`url(#gradAnimated-${i})`} strokeWidth={1.5} />)}
      </svg>
    </div>;
}
