...
const Properties = () => {
  ...
  const handleSearchResults = (search, selects) => {
  ...
  const handleClearFilters = () => {
    ...
    selectNames.forEach(name => {
      const selectInput = document.querySelector(`input[name="${name}"]`);
      if (selectInput) {
        selectInput.value = "";
        // Trigger clear event to update internal state
        selectInput.dispatchEvent(new Event('clear', {
          bubbles: true
        }));
      }
    });
