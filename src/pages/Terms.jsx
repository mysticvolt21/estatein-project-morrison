...
const Terms = () => {
  ...
  const renderContent = content => {
    if (Array.isArray(content)) {
      return content.map((paragraph, index) => <p key={index} className="text-Grey-60 leading-relaxed mb-6 text-base">
          {paragraph}
        </p>);
    }
  ...
  const renderSubsections = subsections => {
    return subsections.map((subsection, index) => <div key={index} className="mb-8">
        <h3 className="text-xl font-semibold mb-4 text-White border-l-4 border-Purple-60 pl-4">
          {subsection.title}
        </h3>
        {Array.isArray(subsection.content) ? <ul className="list-disc list-inside text-Grey-60 space-y-3 ml-6">
            {subsection.content.map((item, itemIndex) => <li key={itemIndex} className="leading-relaxed">{item}</li>)}
          </ul> : <p className="text-Grey-60 leading-relaxed">{subsection.content}</p>}
      </div>);
  };
  return <div className="min-h-screen bg-Grey-08 text-White mt-[68px] lg:mt-[77px] xl:mt-[99px]">
        ...
        <div>
          <div className="bg-Grey-10 rounded-2xl p-8 md:p-12 shadow-2xl border border-Grey-15">
            <div className="space-y-12">
              {termsData.map((section, index) => <section key={section.id} className="group" data-aos="fade-up" data-aos-delay={index * 20}>
                  <div className="flex items-start space-x-4 mb-6">
                    ...
                    <div className="flex-1">
                      ...
                      {section.subsections ? <div className="space-y-6">
                          ...
                          {renderContent(section.content)}
                          {section.id === "privacy" && <div className="mt-6 p-4 bg-Grey-15 rounded-lg border border-Grey-20">
              ...
              <div className="text-center pt-8 border-t border-Grey-15 text-white">
                <Link data-aos="zoom-in" to="/" className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-Purple-60 to-Purple-65 text-White rounded-xl hover:from-Purple-65 hover:to-Purple-70 transition-all duration-300 font-semibold  shadow-lg hover:shadow-xl transform hover:-translate-y-1">
