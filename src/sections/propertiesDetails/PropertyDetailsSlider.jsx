...
function PropertyDetailsSlider({
  ...
  const currentImages = images.slice(currentIndex, currentIndex + imagesPerSlide);
  const ImagesData = data.PropertyImages ? data.PropertyImages : [];
  ...
  return <div className="flex flex-col gap-7.5 md:gap-10 xl:gap-12.5 ">
            ...
            return <img key={index} src={img.image} className={`${isMobile ? 'w-full' : 'w-[48.5%]'} xl:h-[583px] md:h-[507px] h-[250px] object-cover rounded-[10px]`} alt={`Property image ${index + 1}`} style={{
              maxWidth: isMobile ? '100%' : '48.5%'
            }} />;
          })}
                    </div>
                </div>
            ...
            return <span key={i} className={`w-[11.7px] h-[3px] md:w-[20px] md:h-[5px] rounded transition-colors duration-200 ${isActive ? 'bg-Purple-60' : 'bg-Grey-30'}`}></span>;
          })}
                    </div>
                    
                    <button onClick={next} className={`w-11 h-11 md:w-14.5 md:h-14.5 md:p-3.5 p-2.5 border border-Grey-15
                        rounded-full flex  justify-center items-center cursor-pointer
