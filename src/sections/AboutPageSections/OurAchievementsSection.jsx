...
const OurAchievementsSection = ({
  ...
  return <Container>
      <Header hash="works" title={data.headline} description={data.description} withIcon />
      <div className="flex flex-col lg:flex-row gap-y-5 justify-between">
        {loading ? Array.from({
        length: 3
      }).map((_, i) => <FAQCardSkeleton key={i} />) : items.length > 0 ? items.map((item, index) => <FAQCard key={item.id} faq={{
        id: item.id,
        question: item.title,
        answer: item.description,
        readMoreLink: "#",
        btnText: ""
      }} data-aos="fade-up" data-aos-delay={index * 200} withShadow={true} showButton={false} />) : Array.from({
        length: 3
