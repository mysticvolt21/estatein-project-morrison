...
const OurTeam = ({
  ...
  return <Container>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 xl:gap-7.5'>
        {loading ? Array.from({
        length: 4
      }).map((_, i) => <OurTeamCardSkeleton key={i} />) : items.length > 0 ? items.map((item, index) => <OurTeamCard key={item.id} item={item} data-aos="fade-up" data-aos-delay={index * 200} />) : Array.from({
        length: 3
      }).map((_, i) => <OurTeamCardSkeleton key={i} />)}
      </div>
    </Container>;
};
