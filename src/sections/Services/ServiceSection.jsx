...
const ServiceSection = ({
  ...
  return <Container className={`${investSection ? "lg:flex-row " : ""}`}>
        <div className={`${investSection ? "flex flex-col gap-7.5 md:gap-10 xl:gap-12.5 w-full lg:w-[28.6112vw] xl:w-[27.03125vw]" : "w-full lg:w-[67.5vw]"}`}>            
            {investSection ? <Header title={title} description={description} withIcon textVW hash={title.replaceAll(" ", "_").replaceAll(",", "")} /> : <Header title={title} description={description} withIcon hash={title.replaceAll(" ", "_").replaceAll(",", "").toLowerCase()} />}
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 
            ...
            {data.map((item, index) => <ValuesCard key={index} withIcon iconURL={item.icon} title={item.title} description={item.description} data-aos="zoom-out" data-aos-delay={index * 100} />)}
