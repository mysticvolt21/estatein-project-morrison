...
function SearchResultsSection({
  ...
  const filtered = useMemo(() => {
    ...
    const withinYear = (yearStr, label) => {
  ...
  return <Container>
            <div className="-mt-10 md:-mt-20 xl:-mt-35">
                {error && <div className="text-center mb-8">
                        <p className="text-Red-60 text-lg">Error: {error}</p>
                    </div>}

                {loading || isSearching ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                        {Array.from({
          length: 6
        }).map((_, i) => <PropertiesCardSkeleton key={i} />)}
                    </div> : filtered.length === 0 ? <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 -z-1">
                            {currentItems.map(property => <PropertiesCard key={property.id} image={property.imageUrls?.[0]} title={property.name} fullDescription={property.feature_description} price={"$".concat(Number(property.price ?? 0).toLocaleString())} discoverDescription={property.tag_description} showInfo={false} btnLink={`/properties/${property.id}`} btnText={"View Property Details"} />)}
                        </div>

                        {totalPages > 1 && <div className="mt-12 flex flex-col items-center gap-6">
                                {/* Page Info */}
                ...
                const shouldShow = page === 1 || page === totalPages || page === currentPage || Math.abs(page - currentPage) <= 1;
                if (!shouldShow) {
                ...
                return <button key={page} onClick={() => handlePageChange(page)} className={`relative px-3 py-2 md:px-4 md:py-2.5 rounded-lg border transition-all duration-200 ${currentPage === page ? "bg-Purple-60 text-white border-Purple-60 shadow-[0_0_20px_rgba(112,59,247,0.3)] scale-105" : "border-Grey-15 text-Grey-60 hover:bg-Grey-10 hover:border-Purple-60/30 hover:text-Purple-75"}`}>
                                                    {page}
                                                </button>;
              })}
                                    </div>
                                    ...
                                    {/* Next Button */}
                                    <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="group flex items-center gap-2 px-4 py-3 md:px-5 md:py-3.5 rounded-xl border border-Grey-15 text-Grey-60 hover:bg-Grey-10 hover:border-Purple-60/30 hover:text-Purple-75 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:border-Grey-15 disabled:hover:text-Grey-60 transition-all duration-200">
                                ...
                                <div className="flex items-center gap-2">
                                    {Array.from({
              length: totalPages
            }, (_, i) => i + 1).map(page => <button key={page} onClick={() => handlePageChange(page)} className={`w-2 h-2 md:w-3 md:h-3 rounded-full transition-all duration-200 ${currentPage === page ? "bg-Purple-60 scale-125 shadow-[0_0_10px_rgba(112,59,247,0.5)]" : "bg-Grey-30 hover:bg-Grey-40"}`} />)}
                                </div>
                            </div>}
                    </>}
            </div>
